% Code written by Tyler Rook and Jonie Moya
% 
% Based on a Genetic Algorithm written by:
%
...
%% This starts the clean-up process.
cla;
while i < size(positions, 1)
    ...
    sectStart = coordTable(e, 2);
        ...
        for h = sectStart:sectFinish
            %display(strcat("The pair being analyzed:_", num2str(distTable(h, 1)), ",__", num2str(distTable(h, 2))));
            connect = [distTable(h, 1), distTable(h, 2)];
            if connect(2) == positions(i)
                display("Found last connection");
                next = size(positions, 1);
            end
        end
...
%% Begin the pure pursuit algorithm.

controller = robotics.PurePursuit('DesiredLinearVelocity', 1, 'LookaheadDistance', 3, 'MaxAngularVelocity', 1);
controller.Waypoints = waypoints;
pose = [startPoint(1) startPoint(2) 0];
dronePoints = [];
[vel, angvel] = controller(pose);
% Calculate distance from final point.
dist = ((endPoint(1) - pose(1))^2 + (endPoint(2) - pose(2))^2)^(1/2);
while dist > 2
    pose(3) = pose(3) + angvel;
    pose(1) = pose(1) + vel*cos(pose(3));
    pose(2) = pose(2) + vel*sin(pose(3));
    % This bit just for showing the path.
    dronePoints = [dronePoints; [pose(1), pose(2)]];
...
hold on
show(map)
plot(dronePoints(:, 1), dronePoints(:, 2), '-o');

%smoothed = fit(waypoints(:, 1), waypoints(:, 2), 'smoothingspline', 'SmoothingParam', 0.07);
%smoothed = spline(waypoints(:, 1), waypoints(:, 2));
%smoothed
%figure;
%hold on
%show(map);
...
%% This part gives the convergence curve.
